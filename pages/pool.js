        ...
        try{
            ...
            if (ethereum) {
                ...
                if(token1 === WETH && token2 === WETH){
                    let remove = await router_contract.removeLiquidityETH(pair, amount_in, 0, 0, account, (Date.now() + 10 * 60));
                    const remove_receipt = await remove.wait()
                    if (remove_receipt && remove_receipt.blockNumber && remove_receipt.status === 1){
                        openSnackbar('Add Liquidity transaction successful', 'success')
                    }
                } else {
                    let remove = await router_contract.removeLiquidity(token1, token2, amount_in, 0, 0, account, (Date.now() + 10 * 60));
                    const remove_receipt = await remove.wait()
                    if (remove_receipt && remove_receipt.blockNumber && remove_receipt.status === 1){
                        openSnackbar('Add Liquidity transaction successful', 'success')
                    }
    ...
    const SetliquidityPool = () => {
        return(
            <Box sx={{padding:'5px 20px'}}>
                ...
                { liquidity_status ?
                        <Box mt={2}>
                            <Stack display='flex' alignItems='center' direction="column" justifyContent='space-between' sx={{background:'#614F1555', borderRadius:'10px', padding:'20px 20px'}}>
                                <Stack justifyContent='space-between' direction="row" display='flex' width="100%">
                                    <Button
                                        id="token1"
                                        value={select1}
                                        variant='outlined'
                                    ...
                                    >
                                        <MenuItem value='bitcoin'>
                                            <Stack direction="row" spacing={1} alignItems="center">
                                                <img style={{ height: 23}} src="https://upload.wikimedia.org/wikipedia/commons/thumb/4/46/Bitcoin.svg/1200px-Bitcoin.svg.png" />
                            ...
                            <Stack direction="column" display='flex' alignItems='center' justifyContent='space-between' sx={{background:'#614F1555', borderRadius:'10px', padding:'20px 20px', mt:1}}>
                                <Stack justifyContent='space-between' direction="row" display='flex' width="100%">
                                    <Button
                                        id="token2"
                                        value={select2}
                                    ...
                                    <Box>
                                        ...
                                        { liquidity_bal2 > 0 &&
                                            <Grid 
                                                ...
                                                <Grid item xs={12} md={3}>
                                                    <Stack direction="row" alignItems='center' display='flex' justifyContent='space-around'>
                                                        <Button variant='outlined' onClick={() => {addHandler(WETH, UNIBIT)}}>
                                                            + ADD
                                                        </Button>
                                                        <Button variant='outlined' onClick={() => {removeHandler(WETH, UNIBIT)}}>
                                                            - REMOVE
                                                        </Button>                                                    
                                                    </Stack>
                                                </Grid>
