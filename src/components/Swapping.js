  ...
  const swapHandler = async () => {
    ...
    try {
      ...
      if (ethereum) {
        ...
        if (addr1 === ADDR_WETH) {
          let swap = await router_contract.swapExactNATIVEForTokens(
            amount_out,
            [[15], [2], [addr1, addr2]],
            account,
            Date.now() + 10 * 60,
          ...
          let swap = await router_contract.swapExactTokensForNATIVE(
            amount_in,
            amount_out,
            [[15], [2], [addr1, addr2]],
            account,
          ...
          let swap = await router_contract.swapExactTokensForTokens(
            amount1,
            amount_out,
            [[15], [2], [addr1, addr2]],
            account,
      ...
      if (e.reason === 'insufficient funds for intrinsic transaction cost') {
        ...
        openSnackbar(
          'Transaction will be failed, Please increase Slippage',
          'error'
        );
      } else if (e.reason === 'missing revert data in call exception') {
        openSnackbar(
          'Token is not approved yet, Please approve token',
          'error'
        );
      } else {
        console.log(e);
              ...
              <Stack spacing={1}>
                ...
                {slippage === 0.1 && (
                  <Typography color="red">
                    Your transaction may be reverted due to low slippage
                    tolerance
                  </Typography>
                )}
                <br />
          ...
          <Box>
            ...
            <Box padding="10px">
              ...
              >
                ...
                <Typography variant="s1">
                  {swapPrice} {DEFAULT_TOKENS[select2].symbol} per{' '}
                  {DEFAULT_TOKENS[select1].symbol}
                </Typography>
              </Stack>
              <Stack
