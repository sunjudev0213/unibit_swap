...
// Material
import { Avatar, Autocomplete, CardMedia, IconButton, InputAdornment, Link, MenuItem, Stack, TextField, Tooltip, Typography } from "@mui/material";
...
const RenderOption = ({ uuid, meta, dfile, option_type, NFTokenID, logoImage, logo, account, name, verified, items, type, slug }) => {
    const [hLink, setHLink] = useState("");
    const [isVideo, setIsVideo] = useState(false);

    return (
        <Link color="inherit" underline="none" href={hLink}>
            <MenuItem sx={{ pt: 1, pb: 1 }}>
                <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    {option_type === "ACCOUNTS" && <Typography variant="s7">{account}</Typography>}
                </Stack>
            </MenuItem>
        </Link>
    );
