  ...
  const getDetailsHander = async () => {
    try {
      ...
      setRecord({
        ...
        apr: rates[rr[1]].rate / MULTIPLYER,
        period: rates[rr[1]].period,
        endsOn: format(new Date(parseInt(lockTime.toString()) * 1000), "MMM do, yyyy"),
        rewards: ethers.utils.formatEther(rr[3]),
        available: rr[4]
      });
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <TableCell align="right">
        <Button variant='outlined' onClick={claimHandler} disabled={record?.available === 'false'}>Claim and unstake</Button>
      </TableCell>
    </TableRow>
  )
}
