...
export function fNumber(num) {
  ...
  if (strNum.includes('e')) {
    if (strNum.length > 5) return Number(strNum).toExponential(5);
    return num;
  }

  if (intNum.toString().length > 10) return Number(strNum).toExponential(5);
...
export function limitNumber(number) {
  const res = numeral(number).format(
...
export function fPercent(number) {
  if (number < 1) return fp(number);
  else {
    const strNum = number.toFixed(0).trim();
    if (strNum.length > 5) return Number(number).toExponential(0);

    const res = numeral(number).format(
      Number.isInteger(number) ? '0,0' : '0,0.0'
    );
    if (res === 'NaN') return 0;
    return res;
  }
